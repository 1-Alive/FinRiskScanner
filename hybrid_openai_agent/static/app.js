...
const singleSource = document.getElementById("single-source");
const singleModel = document.getElementById("single-model");
const singleRule = document.getElementById("single-rule");
const singleReason = document.getElementById("single-reason");
...
singleForm.addEventListener("submit", async (event) => {
  ...
  singleSource.textContent = "-";
  singleModel.textContent = "-";
  singleRule.textContent = "-";
  singleReason.textContent = "-";

  const response = await fetch("/api/classify", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ description: singleDescription.value }),
  });

  const data = await response.json();
  singleResult.textContent = data.result || "-";
  singleSource.textContent = data.source || "-";
  singleModel.textContent = data.model_used || "-";
  singleRule.textContent = data.rule_path || "-";
  singleReason.textContent = data.reason || "-";
});
